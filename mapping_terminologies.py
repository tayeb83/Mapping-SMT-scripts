...
def run_lexical_mapping(input_termino1_path, input_termino2_path, output_path, language='auto', match_most_s=True,
    ...
    master_id = pd.Series([i for i in range(len(labels_t1))])

    if match_most_s:
        duplicates_id = pd.Series([i for i in range(len(labels_t2))])
        alignement_matching_most_similar = match_most_similar(df_termino1['proc_labels_t1'],
                                                              df_termino2['proc_labels_t2'],
                                                              master_id,duplicates_id,
                                     ngram_size=3, min_similarity=0.8)

        #print(alignement_matching_most_similar.columns)
...
if __name__ == "__main__":
    ...
    #run_lexical_mapping('data/terminos_test/termino1.csv', 'data/terminos_test/termino2.csv',
